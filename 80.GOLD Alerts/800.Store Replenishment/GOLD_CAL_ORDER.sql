...
select 'Delivery Order', ecdcexcde "Order nb",  ecdsite "Site", foucnuf "Supplier code", foulibl "Supplier Desc.", 
       ecdetat "Status", ecddcom "Order date", ecddliv "Delivery date", ecddenvoi "Collection date", 
       ecdtrans  etat_envoi, (SELECT SUM(dcdral) FROM cdedetcde WHERE ecdcincde=dcdcincde AND dcdetat=5) "To be delivered (SKU)"
 from cdeentcde, foudgene, ccldetccl
where ecdsite > 30000 AND ecdcfin=foucfin AND ecdcfin=781 AND ecdcexcde=cclcexcde
and ecdetat=5
and not exists (
select *
 from tmc_icsdeol a
where iolcexcde =ecdcexcde)
-- Delivery order not sent to warehouse
UNION 
select 'Delivery Order', ecdcexcde "Order nb",  ecdsite "Site", foucnuf "Supplier code", foulibl "Supplier Desc.", 
       ecdetat "Status", ecddcom "Order date", ecddliv "Delivery date", ecddenvoi "Collection date", 
       ecdtrans  etat_envoi, (SELECT SUM(dcdral) FROM cdedetcde WHERE ecdcincde=dcdcincde AND dcdetat=5) "To be delivered (SKU)"
 from cdeentcde, foudgene, hccldetccl
where ecdsite > 30000 AND ecdcfin=foucfin AND ecdcfin=781 AND ecdcexcde=cclcexcde
and ecdetat=5
and not exists (
select *
